        ...
        <div class="p-8">
            ...
            <div class="mb-6">
                <form method="GET" class="flex gap-4">
                    <input type="text" name="search" value="<?= htmlspecialchars($search ?? '') ?>" 
                           placeholder="Buscar por título, excerpt ou conteúdo..."
                    ...
                    <?php if ($search): ?>
                        <a href="<?= base_url('admin/blog') ?>" class="bg-azul-medio hover:bg-azul-medio/80 text-white px-6 py-2 rounded-lg transition">
                            Limpar
                        </a>
                    <?php endif; ?>
                </form>
                                ...
                                <tr class="border-b border-azul-medio/50 hover:bg-azul-medio/20 transition">
                                    <td class="py-4 px-6">
                                        <div class="flex items-center gap-4">
                                            ...
                                            <?php endif; ?>
                                            <div>
                                                <div class="text-white font-semibold"><?= htmlspecialchars($post['title']) ?></div>
                                                <div class="text-sm text-cinza-azulado"><?= $post['slug'] ?></div>
                                                <div class="text-xs text-cinza-azulado mt-1 line-clamp-1"><?= htmlspecialchars($post['excerpt']) ?></div>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4">
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex gap-2 justify-end">
                                            ...
                                            <form method="POST" action="<?= base_url("admin/blog/{$post['id']}/deletar") ?>" 
                                                  onsubmit="return confirm('Tem certeza que deseja deletar este post?');" class="inline">
            ...
            <?php if (isset($pagination) && $pagination['total_pages'] > 1): ?>
                <div class="mt-6 flex justify-center gap-2">
                    <?php for ($i = 1; $i <= $pagination['total_pages']; $i++): ?>
                        <a href="<?= base_url('admin/blog?page=' . $i . ($search ? '&search=' . urlencode($search) : '')) ?>" 
            ...
            <?php else: ?>
                <div class="text-center py-12">
                    ...
                    <a href="<?= base_url('admin/blog/criar') ?>" class="inline-block mt-4 bg-dourado-luz hover:bg-dourado-intenso text-azul-noite px-6 py-3 rounded-lg font-semibold transition">
                        Criar Primeiro Post
                    </a>
                </div>
            <?php endif; ?>

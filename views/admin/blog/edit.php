    ...
    <div class="flex-1 overflow-auto">
        <div class="p-8">
            <div class="mb-8">
                ...
                <h1 class="text-3xl font-bold text-dourado-luz mt-4">Editar Post</h1>
            </div>
            ...
            <form method="POST" action="<?= base_url("admin/blog/{$post['id']}/editar") ?>" enctype="multipart/form-data" class="space-y-6">
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <textarea name="excerpt" rows="3" required
                                      class="w-full px-4 py-3 bg-azul-noite border border-azul-medio rounded-lg text-white focus:outline-none focus:border-dourado-luz"
                                      maxlength="300"><?= htmlspecialchars($post['excerpt']) ?></textarea>
                    ...
                    <div class="space-y-6">
                        <div>
                            ...
                            <div class="grid grid-cols-2 gap-3">
                                <?php if (!empty($categories)): ?>
                                    <?php 
                                    $selectedCategories = array_filter($postTaxonomies, function($t) { return $t['taxonomy_type'] === 'category'; });
                                    ...
                                    <?php foreach ($categories as $category): ?>
                                        <label class="flex items-center gap-2 text-white cursor-pointer hover:text-dourado-luz transition">
                                            <input type="checkbox" name="categories[]" value="<?= $category['id'] ?>" 
                                                   ...
                                                   class="w-4 h-4 rounded border-azul-medio bg-azul-noite text-dourado-luz focus:ring-dourado-luz">
                        ...
                        <div>
                            ...
                            <div class="grid grid-cols-2 gap-3">
                                <?php if (!empty($tags)): ?>
                                    <?php 
                                    $selectedTags = array_filter($postTaxonomies, function($t) { return $t['taxonomy_type'] === 'tag'; });
                ...
                <div class="bg-azul-cosmico rounded-lg border border-azul-medio p-8">
                    ...
                    <div class="space-y-6">
                        <div>
                            ...
                            <input type="text" value="<?= htmlspecialchars($post['slug']) ?>" disabled
                                   class="w-full px-4 py-3 bg-azul-noite/50 border border-azul-medio rounded-lg text-cinza-azulado">
                            <p class="text-sm text-cinza-azulado mt-2">O slug é gerado automaticamente a partir do título</p>
                        </div>
                        
                        <div>
                            <label class="block text-dourado-luz font-semibold mb-2">Título SEO</label>
                            <input type="text" name="seo_title" value="<?= htmlspecialchars($post['seo_title'] ?? '') ?>"
                                   class="w-full px-4 py-3 bg-azul-noite border border-azul-medio rounded-lg text-white focus:outline-none focus:border-dourado-luz">
                        ...
                        <div>
                            ...
                            <textarea name="seo_meta_description" rows="3"
                                      class="w-full px-4 py-3 bg-azul-noite border border-azul-medio rounded-lg text-white focus:outline-none focus:border-dourado-luz"
                                      maxlength="160"><?= htmlspecialchars($post['seo_meta_description'] ?? '') ?></textarea>
                        </div>
                        
                        <div>
                            <label class="block text-dourado-luz font-semibold mb-2">Palavras-chave</label>
                            <input type="text" name="seo_keywords" value="<?= htmlspecialchars($post['seo_keywords'] ?? '') ?>"
                                   class="w-full px-4 py-3 bg-azul-noite border border-azul-medio rounded-lg text-white focus:outline-none focus:border-dourado-luz">
...
tinymce.init({
    ...
    setup: function(editor) {
        ...
        editor.on('init', function() {
            var content = <?= json_encode($post['content_html'] ?? '') ?>;
            if (content) {
